...
export function CourseMap(props: CourseMapProps) {
  ...
  useEffect(() => {
    ...
    loadSdk().then(api => {
      ...
      if (places.length > 1) { map.fitBounds(bounds); if (props.showRoute !== false) new api.Polyline({ map, path: places.map(p => new api.LatLng(p.latitude, p.longitude)), strokeColor: '#2AA484', strokeOpacity: .8, strokeWeight: 4 }); }
