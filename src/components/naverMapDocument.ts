...
export type CourseMapProps = { record: OutingRecord; selectedPlaceId?: string; onSelectPlace?: (id: string) => void; showRoute?: boolean; detailTheme?: boolean };
...
export function mapDocument(record: OutingRecord, clientId: string, showRoute = true) {
  ...
  if(places.length>1){map.fitBounds(bounds);if(${showRoute})new naver.maps.Polyline({map,path:places.map(p=>new naver.maps.LatLng(p.latitude,p.longitude)),strokeColor:'#F2789F',strokeOpacity:.7,strokeWeight:4});}
