...
export function mapDocument(record: OutingRecord, clientId: string) {
  ...
  if(places.length>1){map.fitBounds(bounds);new naver.maps.Polyline({map,path:places.map(p=>new naver.maps.LatLng(p.latitude,p.longitude)),strokeColor:'#F2789F',strokeOpacity:.7,strokeWeight:4});}
