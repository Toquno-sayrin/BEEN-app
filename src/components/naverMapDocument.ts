...
export function mapDocument(record: OutingRecord, clientId: string, showRoute = true) {
  ...
  if(places.length>1){map.fitBounds(bounds);if(${showRoute})new naver.maps.Polyline({map,path:places.map(p=>new naver.maps.LatLng(p.latitude,p.longitude)),strokeColor:'#2AA484',strokeOpacity:.8,strokeWeight:4});}
